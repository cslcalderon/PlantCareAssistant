...
from utils import merge_sort, filter_out_list


def deal_with_option(option, plant_trie, plants_dict):
    if option == "🪴 Add Plant":
        add_a_plant_option(plant_trie)
    elif option == "📝 Schedule Task":
        schedule_task_options()
    elif option == "👩‍🌾 My Garden":
        view_garden_option(plants_dict)
    elif option == "🗓️ View Task Schedule":
        view_task_schedule()
    else:
        view_all_tasks()


def add_a_plant_option(plant_trie):
    st.write("If you see a plant that matches your plant or is similar enough to it, please click on the respective button.")
    user_plant_input = st.text_input("Enter the type of plant you are interested in (e.g., 'tree')")
    if user_plant_input:
...
def view_garden_option(plants_dict):
    st.header("👩‍🌾 My Garden")
